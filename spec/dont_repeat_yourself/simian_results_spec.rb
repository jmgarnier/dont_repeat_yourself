...
END_OF_STRING
  ...
  describe "(after parsing)" do
    ...
    before :each do       
      yaml_results = YAML.load( read_simian_log("full_simian_log.yaml") )
    ...
    # FIXME what's the added value of this matcher ???
    def have_attribute(attribute, value)
      return simple_matcher("have attribute '#{attribute}' ")  do |results| 
        (results.send(attribute)) == value
      end        
    end
    
    it "should generate the #{""}" do 
    ...
    it "should generate the #{"sentence_processed_a_total_of_x_significant_lines_in_y_files"}" do 
    ...
    describe DontRepeatYourself::SimianResults::DuplicateLinesSet do
      ...
      it "should generate the #{"sentence_found_x_duplicate_lines_in_the_following_files"}" do 
    ...
    describe DontRepeatYourself::SimianResults::DuplicateLinesBlock do
      ...
      it "should generate the #{"sentence_between_lines_x_and_y_in_filepath"}" do 
